...
class QuizApp:
    ...
    def init_quiz(self):
        ...
        self.question_num = 0
        self.guesses = []
        self.hints_used = [False] * len(quiz_data)  # Track whether hint was used for each question
    ...
    def display_question(self):
        ...
        self.canvas.create_window(300, 390, window=self.submit_button)

        self.hint_button = tk.Button(self.root, text="Hint", command=self.show_hint)
        self.canvas.create_window(500, 390, window=self.hint_button)

    def show_hint(self):
        hint = quiz_data[self.question_num].get('hint', 'No hint available')
        messagebox.showinfo("Hint", hint)
        self.hints_used[self.question_num] = True  # Mark that hint was used
    ...
    def show_results(self):
        ...
        hints_used_label = tk.Label(result_window, text=f"Hints used: {' '.join(['Yes' if used else 'No' for used in self.hints_used])}", pady=5)
        hints_used_label.pack()
